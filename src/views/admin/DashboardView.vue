    ...
    <div class="space-y-6">
      ...
      <div class="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        ...
        <div v-else class="divide-y divide-gray-50">
          ...
          >
            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" :class="fileIconBg(doc.file_type)">
              <svg class="w-4 h-4" :class="fileIconColor(doc.file_type)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>

            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900 truncate">{{ doc.filename || doc.name || 'Untitled' }}</p>
              <p class="text-xs text-gray-400 mt-0.5">{{ formatDate(doc.created_at || doc.uploaded_at) }}</p>
            </div>

            <span :class="statusBadge(doc.status)" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium flex-shrink-0">
